...
-- Version SAFE - Gère les tables existantes
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'numero_abonne') THEN
    ALTER TABLE users ADD COLUMN numero_abonne text;
    CREATE UNIQUE INDEX IF NOT EXISTS idx_users_numero_abonne ON users(numero_abonne);
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'statut_abonnement') THEN
    ALTER TABLE users ADD COLUMN statut_abonnement text DEFAULT 'inactif';
  END IF;

  -- Ajouter le check constraint si nécessaire
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'users_statut_abonnement_check'
    AND conrelid = 'users'::regclass
  ) THEN
    ALTER TABLE users ADD CONSTRAINT users_statut_abonnement_check
      CHECK (statut_abonnement IN ('actif', 'inactif', 'expire', 'essai'));
  END IF;
...
-- Ajouter UNIQUE constraint sur user_id si pas déjà présent
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'abonnements_user_id_key'
    AND conrelid = 'abonnements'::regclass
  ) THEN
    ALTER TABLE abonnements ADD CONSTRAINT abonnements_user_id_key UNIQUE (user_id);
  END IF;
END $$;
...
-- Ajouter les colonnes manquantes à paiements progressivement
DO $$
BEGIN
  -- Check constraint pour statut
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    ...
    ALTER TABLE paiements ADD CONSTRAINT paiements_statut_check
      CHECK (statut IN ('en_attente', 'confirme', 'echoue', 'annule', 'rembourse'));
  END IF;

  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'paiements' AND column_name = 'ipay_status') THEN
    ALTER TABLE paiements ADD COLUMN ipay_status text DEFAULT 'pending';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'paiements' AND column_name = 'reference') THEN
    ALTER TABLE paiements ADD COLUMN reference text;
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'paiements' AND column_name = 'country_code') THEN
    ALTER TABLE paiements ADD COLUMN country_code text DEFAULT 'SN';
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'paiements' AND column_name = 'confirmed_by') THEN
    ALTER TABLE paiements ADD COLUMN confirmed_by uuid REFERENCES auth.users(id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'paiements' AND column_name = 'confirmed_at') THEN
    ALTER TABLE paiements ADD COLUMN confirmed_at timestamptz;
  END IF;

  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'paiements' AND column_name = 'metadata') THEN
    ALTER TABLE paiements ADD COLUMN metadata jsonb DEFAULT '{}'::jsonb;
  END IF;
END $$;

-- Ajouter UNIQUE constraint sur reference si pas déjà présent
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'paiements_reference_key'
    AND conrelid = 'paiements'::regclass
  ) THEN
    -- D'abord nettoyer les doublons potentiels
    UPDATE paiements SET reference = NULL WHERE reference IS NOT NULL AND reference IN (
      SELECT reference FROM paiements WHERE reference IS NOT NULL GROUP BY reference HAVING COUNT(*) > 1
    );

    ALTER TABLE paiements ADD CONSTRAINT paiements_reference_key UNIQUE (reference);
  END IF;
END $$;
...
-- Ajouter les check constraints sur payment_events
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'payment_events_actor_type_check'
    AND conrelid = 'payment_events'::regclass
  ) THEN
    ALTER TABLE payment_events ADD CONSTRAINT payment_events_actor_type_check
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'pdfs' AND column_name = 'statut_publication') THEN
    ALTER TABLE pdfs ADD COLUMN statut_publication text DEFAULT 'brouillon';
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'pdfs_statut_publication_check'
    AND conrelid = 'pdfs'::regclass
  ) THEN
    ALTER TABLE pdfs ADD CONSTRAINT pdfs_statut_publication_check
...
CREATE INDEX IF NOT EXISTS idx_paiements_reference ON paiements(reference) WHERE reference IS NOT NULL;
